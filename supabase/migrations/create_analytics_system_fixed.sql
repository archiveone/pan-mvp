...
BEGIN
  CASE field_name
    WHEN 'view_count' THEN
      UPDATE posts SET view_count = view_count + 1 WHERE id = content_id;
    WHEN 'like_count' THEN
      UPDATE posts SET like_count = like_count + 1 WHERE id = content_id;
    WHEN 'comment_count' THEN
      UPDATE posts SET comment_count = comment_count + 1 WHERE id = content_id;
    WHEN 'share_count' THEN
      UPDATE posts SET share_count = share_count + 1 WHERE id = content_id;
    WHEN 'download_count' THEN
      UPDATE posts SET download_count = download_count + 1 WHERE id = content_id;
    WHEN 'play_count' THEN
      UPDATE posts SET play_count = play_count + 1 WHERE id = content_id;
    WHEN 'save_count' THEN
      UPDATE posts SET save_count = save_count + 1 WHERE id = content_id;
    WHEN 'click_count' THEN
      UPDATE posts SET click_count = click_count + 1 WHERE id = content_id;
  END CASE;
END;
$$ LANGUAGE plpgsql;
