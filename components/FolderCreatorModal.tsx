...
import { FoldersService, FOLDER_COLORS, FOLDER_CATEGORIES, CreateFolderData } from '@/services/foldersService';
...
const FolderCreatorModal: React.FC<FolderCreatorModalProps> = ({
  isOpen,
  onClose,
  onFolderCreated,
  userId
}) => {
  const [step, setStep] = useState<'category' | 'customize'>('category');
  ...
  const handleCategorySelect = (category: string, icon: string) => {
    const categoryInfo = FOLDER_CATEGORIES.find(c => c.value === category);
    setFolderData({
      ...folderData,
      category,
      icon,
  ...
  const handleCreate = async () => {
    ...
    setLoading(true);
    try {
      const result = await FoldersService.createFolder(userId, folderData);
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-2 sm:p-4 overflow-y-auto animate-in fade-in duration-200">
      <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl w-full max-w-2xl my-auto max-h-[calc(100vh-1rem)] sm:max-h-[90vh] overflow-y-auto">
        {/* Header */}
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                {step === 'category' ? 'Choose a category' : 'Customize your folder'}
              </p>
            </div>
            <button
              onClick={onClose}
          ...
          {step === 'category' ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {FOLDER_CATEGORIES.map((category) => {
                ...
                return (
                  <button
                    key={category.value}
                    onClick={() => handleCategorySelect(category.value, category.icon)}
                    className="group relative p-6 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-700 dark:to-gray-800 rounded-2xl hover:shadow-xl transition-all duration-300 hover:scale-105 active:scale-95"
                  >
                    <div className="flex flex-col items-center gap-3">
                      <div className="p-4 bg-white dark:bg-gray-900 rounded-xl group-hover:scale-110 transition-transform">
            ...
            <div className="space-y-6">
              ...
              <ColorPicker
                ...
                colorType={folderData.color_type || 'preset'}
                  ...
                  <div className="absolute inset-0 p-6 flex flex-col justify-end">
                    <div className="flex items-start justify-between">
                      ...
                      <div className="bg-white/20 backdrop-blur-sm rounded-xl p-2.5 ml-3 flex-shrink-0">
                        {ICON_MAP[folderData.icon || 'Folder'] && 
                          React.createElement(ICON_MAP[folderData.icon || 'Folder'], { className: 'w-6 h-6 text-white' })
                        }
                      </div>
                    </div>
                  </div>
        ...
        <div className="p-6 border-t border-gray-200 dark:border-gray-700 flex gap-3">
          ...
          <button
            onClick={onClose}
            className="flex-1 px-6 py-3 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-xl hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors font-medium"
          >
            Cancel
          </button>
          {step === 'customize' && (
            <button
              ...
              disabled={loading || !folderData.title.trim()}
              className="flex-1 px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all font-medium flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? (
                <>
